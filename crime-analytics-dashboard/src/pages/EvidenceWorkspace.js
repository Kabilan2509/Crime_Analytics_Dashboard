...
function EvidenceWorkspace() {
  ...
  const handleDownloadFile = (item) => {
    ...
    if (extension === 'pdf') {
      downloadPdf(item.name, 'KSP Digital Evidence Record', detailLines);
  ...
  const handleExportChainOfCustody = () => {
    ...
    downloadPdf(`court-evidence-package-${activeCase?.CaseMasterID || caseId || 'case'}-${Date.now()}.pdf`,
      ...
      [
        `Generated: ${courtPackage.generatedAt}`, `Generated by: ${courtPackage.generatedBy}`,
        `Case Master ID: ${courtPackage.caseMasterId || ''}`, `Evidence count: ${courtPackage.evidenceCount}`, '',
        ...courtPackage.evidence.flatMap(item => [
          `${item.id} | ${item.name} | ${item.type} | ${item.hash}`,
          ...(item.chainOfCustody || []).map(entry => `  ${entry.date} | ${entry.operator} | ${entry.action}`), ''
        ])
      ]);
  };
