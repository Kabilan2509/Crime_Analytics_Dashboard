...
function EvidenceWorkspace() {
  ...
  const handleDownloadFile = (item) => {
    ...
    const manifest = {
      packageType: 'KSP Evidence Manifest',
      generatedAt: downloadedAt,
      caseMasterId: activeCase?.CaseMasterID || caseId || null,
      evidence: {
        ...item,
        chainOfCustody: [
          ...(item.chainOfCustody || []),
          {
            date: downloadedAt,
            operator: session.officerName || 'Authorized dashboard user',
            action: 'Tamper-evident evidence manifest downloaded',
            hash: item.hash,
          },
        ],
      },
    ...
    const detailLines = [
      `Evidence ID: ${item.id}`, `Case Master ID: ${manifest.caseMasterId || ''}`,
      `Type: ${item.type}`, `Collected: ${item.date}`, `Collected by: ${item.collectedBy}`,
      `Device: ${item.device || ''}`, `Hash: ${item.hash}`, `Status: ${item.status}`,
      '', item.aiDetails?.ocr || 'No text extraction is available.', '', 'Chain of Custody:',
      ...(manifest.evidence.chainOfCustody || []).map(entry => `${entry.date} | ${entry.operator} | ${entry.action} | ${entry.hash || ''}`)
    ];
    const extension = item.name.split('.').pop().toLowerCase();
    if (extension === 'pdf') {
      downloadPdf(item.name, 'KSP Digital Evidence Record', {
        metadata: [
          { label: 'Evidence ID', value: item.id }, { label: 'Case Master ID', value: manifest.caseMasterId || 'Not assigned' },
          { label: 'Evidence type', value: item.type }, { label: 'Status', value: item.status },
        ],
        sections: [
          { heading: 'Evidence Details', keyValues: [
            { label: 'File name', value: item.name }, { label: 'Collected', value: item.date },
            { label: 'Collected by', value: item.collectedBy }, { label: 'Device', value: item.device || 'Not recorded' },
            { label: 'Integrity hash', value: item.hash },
          ] },
          { heading: 'Forensic Notes', paragraphs: [item.aiDetails?.ocr || 'No text extraction is available.'] },
          { heading: 'Chain of Custody', table: {
            headers: ['Date and time', 'Operator', 'Action', 'Integrity hash'],
            rows: (manifest.evidence.chainOfCustody || []).map(entry => [entry.date, entry.operator, entry.action, entry.hash || item.hash]),
          } },
        ],
      });
    } else if (extension === 'csv') {
      downloadCsv(item.name, ['Field', 'Value'], detailLines.map((line, index) => [index + 1, line]));
    } else if (extension === 'jpg' || extension === 'jpeg') {
      downloadJpegSummary(item.name, 'KSP Evidence Image Record', detailLines);
    } else {
      // The dashboard stores metadata for media assets, not their source binary.
      downloadJsonFile(`${item.name}.evidence.json`, manifest);
    }
  };
  ...
  const handleExportChainOfCustody = () => {
    ...
    const courtPackage = {
      packageType: 'KSP Court Evidence Package',
      generatedAt: exportedAt,
      generatedBy: session.officerName || 'Authorized dashboard user',
      caseMasterId: activeCase?.CaseMasterID || caseId || null,
      ...
      evidence: items.map(item => ({
        ...
        chainOfCustody: [
          ...(item.chainOfCustody || []),
          {
            date: exportedAt,
            operator: session.officerName || 'Authorized dashboard user',
            action: 'Included in Court Evidence Package export',
            hash: item.hash,
          },
        ],
      })),
    };
    downloadPdf(`court-evidence-package-${activeCase?.CaseMasterID || caseId || 'case'}-${Date.now()}.pdf`,
      'KSP Court Evidence Package - Chain of Custody', {
        orientation: 'landscape',
        metadata: [
          { label: 'Generated', value: courtPackage.generatedAt }, { label: 'Generated by', value: courtPackage.generatedBy },
          { label: 'Case Master ID', value: courtPackage.caseMasterId || 'Not assigned' }, { label: 'Evidence count', value: courtPackage.evidenceCount },
        ],
        sections: [
          { heading: 'Evidence Register', table: {
            headers: ['Evidence ID', 'File name', 'Type', 'Status', 'Integrity hash'],
            rows: courtPackage.evidence.map(item => [item.id, item.name, item.type, item.status, item.hash]),
          } },
          { heading: 'Complete Chain of Custody', table: {
            headers: ['Evidence ID', 'Date and time', 'Operator', 'Custody action', 'Integrity hash'],
            rows: courtPackage.evidence.flatMap(item => (item.chainOfCustody || []).map(entry => [item.id, entry.date, entry.operator, entry.action, entry.hash || item.hash])),
          } },
        ],
      });
  };
